...
function cronToChinese(cronExp) {
    ...
    const result = [];
    const reg = /^((?<s>\S+)\s+)?(?<m>\S+)\s+(?<h>\S+)\s+(?<d>\S+)\s+(?<M>\S+)\s+(?<w>\S+)\s*(\s+(?<y>\S+)\s*)?$/;
    const { s: second, m: minute, h: hour, d: day, M: month, w: week, y: year, } = cronExp.match(reg)?.groups || {};
    const isAny = {
    ...
    const unit_month = month
        .toUpperCase()
        .replace(new RegExp(`(${Object.keys(MonthEnum).join("|")})`, "g"), (match) => {
        return MonthEnum[match];
    });
    const unit_week = week
        .toUpperCase()
        .replace(new RegExp(`(${Object.keys(WeekEnum).join("|")})`, "g"), (match) => {
        return WeekEnum[match];
    });
    console.log(`'时间解构结果': 秒：${second}, 分钟：${minute}, 小时：${hour}, 天：${day}, 月： ${month}, 星期：${week}, 年：${year}`);
    // 检查是否所有字段都是*
    ...
    if (!isAny.month) {
        if (month.includes("/")) {
            ...
            result.push(`${isAny.year || year.length > 4 ? "每年" : ""}${unit_month.replaceAll(",", "、")}${CRON_TIME_CN[4]}`);
        }
    }
    // 解析周
    if (!isAny.week) {
        if (unit_week.includes(",")) {
            result.push(`每${CRON_TIME_CN[5]}的${unit_week
                .split(",")
            ...
            result.push(`${isAny.month || unit_month.match(/,|-|\//) ? "每月" : ""}最后一个${CRON_TIME_CN[5]}${weekName}`);
        }
        else if (unit_week.includes("-")) {
            const [start, end] = unit_week.split("-");
            const weekOne = judgeWeek(start);
            const weekTwo = judgeWeek(end);
            result.push(`每${CRON_TIME_CN[5]}的${CRON_TIME_CN[5] + weekOne}到${CRON_TIME_CN[5] + weekTwo}`);
        }
        else {
    ...
    if (!isAny.minute) {
        ...
        if (minute.includes("/")) {
            ...
            result.push(`${suffix}${start}${CRON_TIME_CN[1]}到${end}${CRON_TIME_CN[1]}`);
        }
        else {
    ...
    if (!isAny.second) {
        if (second.includes("/")) {
            ...
            result.push(`${suffix_second}${second !== "0"
